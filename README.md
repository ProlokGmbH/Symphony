...
Zusätzlich überspringt ein abgeschlossener `Review (AI)` ohne Findings den Freigabepunkt `Freigabe Review` nur dann automatisch, wenn der Workspace nach dem Review sauber ist, und geht direkt nach `Test (AI)`. Sobald der Review Änderungen hinterlässt oder das No-Findings-Signal nicht eindeutig ist, greift der reguläre Review-Handoff. `--yolo` und `Skip "Freigabe Review"` bleiben harte Skip-Regeln und überspringen `Freigabe Review` auch dann, wenn im Review Findings oder Änderungen vorhanden waren.
...
| `Review (AI)` | AI | Repository-spezifischer Review-/Fix-Zyklus. | ohne Findings und mit sauberem Workspace `Test (AI)`, sonst `Freigabe Review`; `--yolo` und `Skip "Freigabe Review"` überspringen den Freigabepunkt auch bei Findings oder Änderungen |
