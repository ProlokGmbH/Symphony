...
Wenn für einen Status ein passendes Label `Skip "<Status>"` gesetzt ist, läuft Symphony direkt zum nächsten nicht übersprungenen Status weiter; das gilt für die Freigabepunkte `Freigabe Implementierung` und `Freigabe Review`. Für `Planung` gibt es kein Skip-Label. Im `--yolo`-Modus gelten nur `Freigabe Implementierung` und `Freigabe Review` als übersprungen. Der `Review (AI)`-Handoff nach Findings, Review-Fixes, Dirty-Workspace oder uneindeutigem No-Findings-Signal wird davon nicht übersteuert.

Zusätzlich überspringt ein abgeschlossener `Review (AI)` ohne Findings den Freigabepunkt `Freigabe Review` nur dann automatisch, wenn der Workspace nach dem Review sauber ist, und geht direkt nach `Test (AI)`. Sobald der Review Findings oder Änderungen hinterlässt, diese Evidenz in separaten Review-/Fix-Kommentaren oder im Workpad-Verlauf dokumentiert ist, Workpad- oder Workspace-Evidenz fehlt oder das No-Findings-Signal nicht eindeutig ist, führt der Review-Handoff nach `Freigabe Review`; `--yolo` und `Skip "Freigabe Review"` überspringen diesen Freigabepunkt in diesem Fall auch in Folgeläufen nicht.
...
| `Review (AI)` | AI | Repository-spezifischer Review-/Fix-Zyklus. | ohne Findings und mit sauberem Workspace `Test (AI)`, sonst `Freigabe Review` |
