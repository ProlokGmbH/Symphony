defmodule SymphonyElixir.AgentRunner do
  ...
  defp resolve_review_handoff_state(%Issue{} = issue, :no_findings, workspace, worker_host) do
    if review_workspace_clean?(issue, workspace, worker_host) do
      resolve_review_no_findings_handoff_state(issue)
    else
      resolve_next_handoff_state(issue)
    end
  end
  ...
  defp resolve_review_no_findings_handoff_state(%Issue{} = issue) do
    labels = [@review_no_findings_skip_label | Issue.label_names(issue)]

    Workflow.resolve_next_status(issue.state, labels) ||
