...
5. Falls Konflikte bestehen, nutze den Skill `symphony-pull`, um
   ...
   `<Issue-Key> Merge (AI) Autocommit` plus kurzem Body, verschiebe das Issue
   zurück nach `Test (AI)` und stoppe, damit der Testzyklus auf dem
   aktualisierten Code erneut läuft. Nur wenn kein erneuter Lauf nötig ist,
   veröffentliche den aktualisierten Branch mit `symphony-push`.
